eel = {
    ...
    _ws: function () {
        ...
        eel._websocket.onclose = function(event) {
            if (event.wasClean) {
                  ...
                  console.error(`Соединение прервано, код=${event.code}`)
              }
        };
    },
    ...
    _init: function() {
        document.addEventListener("DOMContentLoaded", function(event) {
            eel._position_window(eel._page());

            eel._ws()
            let connection_closing = 2
            let connection_closed = 3

            setInterval(() => {
                let ws_connection = eel._websocket.readyState
       
                if (ws_connection === connection_closing || ws_connection === connection_closed){
                    eel._ws()
                }
            }, 1000)
        });
