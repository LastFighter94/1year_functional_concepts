...
import socket
import mimetypes
from single_use.logme import logIt
...
def _static(path):
    response = None
    if 'jinja_env' in _start_args and 'jinja_templates' in _start_args and path.endswith(".html"):
        ...
        if path.startswith(template_prefix):
            ...
            try:
                ...
                print(e)
    if response is None:
        if not os.path.exists(os.path.join(root_path, path)) and ".html" in path:
            path="web/404.html"
...
def send_delayed():
    global _delayed_calls
    while len(_delayed_calls) > 0:
        for _, ws in _websockets:
            _repeated_send(ws, _safe_json(_delayed_calls.pop()))
...
def _process_message(message, ws):
    if 'call' in message:
        ...
        logIt('Invalid message received: ', message)
...
def _js_call(name, args):
    ...
    if _semaphore.acquire(): 
        if len(_websockets) == 0:
            _delayed_calls.append(call_object)
        for _, ws in _websockets:
