<template>
  <div class="wrapper">
      ...
      >
        <template #item="{ element }">
          <div class="items-arr">
            <i class="handle"></i>
            {{ element.name }}
            <button @click="items_arr.pop()">X (example)</button>
          </div>
        </template>
    </draggable>
    ...
    <!-- изменения в состоянии массива тоже происходят -->

    <p>
      <button @click="items_arr.push(
        {
          name: 'kistya', age: 30
        }
      )">
        Test adding to items-list
      </button>
    </p>
